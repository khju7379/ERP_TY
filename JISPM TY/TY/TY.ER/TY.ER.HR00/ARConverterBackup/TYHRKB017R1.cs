...
namespace TY.ER.HR00
{
    ...
    /// </summary>
    public partial class TYHRKB017R1 : DataDynamics.ActiveReports.ActiveReport
    {
        ...
        private string fsSABUN;
        private string fsYDATE;

        public TYHRKB017R1(DataTable dtM, DataTable dtS, DataTable dtY, double dBAESU)
        {
            //
        ...
        private void reportHeader1_Format(object sender, EventArgs e)
        {
            ...
            fsSABUN = _dt.Rows[0]["PSSABUN"].ToString();
            fsYDATE = _dt.Rows[0]["PSYDATE"].ToString();
            ...
            if (_dt.Rows[0]["PSYEAR"].ToString() != "0")
            {
                ...
                sPSSERVPAYAMTYY = string.Format("{0:#,###}", double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString())) + " x " + _dt.Rows[0]["PSYEAR"].ToString() + " = " + string.Format("{0:#,###}",UP_COMP(double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString()) * double.Parse(_dt.Rows[0]["PSYEAR"].ToString()) * fdBAESU, 0)) + "\n";
            }
            if (_dt.Rows[0]["PSMONTH"].ToString() != "0")
            {
                sPSYYMMDD += _dt.Rows[0]["PSMONTH"].ToString() + "개월 ";
                sPSSERVPAYAMTYY += string.Format("{0:#,###}", double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString())) + " x " + _dt.Rows[0]["PSMONTH"].ToString() + " / 12 = " + string.Format("{0:#,###}", UP_COMP(double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString()) * double.Parse(_dt.Rows[0]["PSMONTH"].ToString()) * fdBAESU, 12)) + "\n";
            }
            if (_dt.Rows[0]["PSDAY"].ToString() != "0")
            {
                sPSYYMMDD += _dt.Rows[0]["PSDAY"].ToString() + "일 ";
                //sPSSERVPAYAMTYY += string.Format("{0:#,###}", double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString())) + " x " + _dt.Rows[0]["PSDAY"].ToString() + " / 365 = " + string.Format("{0:#,###}", UP_COMP(double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString()) * double.Parse(_dt.Rows[0]["PSDAY"].ToString()) * fdBAESU, 4380));
                sPSSERVPAYAMTYY += string.Format("{0:#,###}", double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString())) + " x " + _dt.Rows[0]["PSDAY"].ToString() + " / 365 = " + string.Format("{0:#,###}", UP_COMP(double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString()) * double.Parse(_dt.Rows[0]["PSDAY"].ToString()) * fdBAESU, 365));
            }
        ...
        private void UP_setM()
        {
            ...
            if(icount > 0)
            {
                ...
                sEdate = _dtM.Rows[icount-1]["PKPAYYYMM"].ToString();
            }
        ...
        private void UP_setS()
        {
            ...
            if (icount > 15)
            {
                PKPAYGUBNNM16.Text = _dtS.Rows[15]["PKPAYGUBNNM"].ToString();
                PKPAYYYMMS16.Text = UP_DateChange(_dtS.Rows[15]["PKPAYYYMM"].ToString());
                PKPAYAMOUNTS16.Text = string.Format("{0:#,###}", double.Parse(_dtS.Rows[15]["PKPAYAMOUNT"].ToString()));
                PKMEMO16.Text = _dtS.Rows[15]["PKMEMO"].ToString();
                dAMTTOTAL += double.Parse(_dtS.Rows[15]["PKPAYAMOUNT"].ToString());
                sEdate = _dtS.Rows[15]["PKPAYYYMM"].ToString();
            }            

            if (dAMTTOTAL > 0)
            {
                if (fsSABUN == "0064-M" && fsYDATE == "20210115")
                {
                    dAMTTOTAL = 84890300;
                }
                
                PSAVGBONUSAMT.Text = string.Format("{0:#,###}", dAMTTOTAL) + " / 12 = " + string.Format("{0:#,###}", double.Parse(_dt.Rows[0]["PSAVGTOTAL"].ToString()));
