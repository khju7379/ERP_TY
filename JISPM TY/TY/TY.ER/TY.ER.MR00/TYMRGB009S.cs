...
namespace TY.ER.MR00
{
    ...
    ///  Z105013 : 자재명１
    /// </summary>
    public partial class TYMRGB009S : TYBase
    {
        // 비품 마스타
        public string fsMAYYMM   = string.Empty;
        public string fsMASEQ    = string.Empty;
        public string fsMABPDESC = string.Empty;
        ...
        private void TYMRGB009S_Load(object sender, System.EventArgs e)
        {
        ...
        {
            this.FPS91_TY_S_MR_2BLAK554.Initialize();
            ...
            if (dt.Rows.Count > 0)
            {
                this.FPS91_TY_S_MR_2BLAK554.SetValue(dt);
            }
            else
            {
                this.FPS91_TY_S_MR_2BLAK554.SetValue(dt);
        ...
        private void FPS91_TY_S_MR_2BLAK554_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            fsMAYYMM   = this.FPS91_TY_S_MR_2BLAK554.GetValue("MAYYMM").ToString();
            fsMASEQ    = this.FPS91_TY_S_MR_2BLAK554.GetValue("MASEQ").ToString();
            fsMABPDESC = this.FPS91_TY_S_MR_2BLAK554.GetValue("MABPDESC").ToString();

            fsJASAN   = this.FPS91_TY_S_MR_2BLAK554.GetValue("MABPCODE").ToString().Substring(0, 1).ToString();
            fsSAUPBU  = this.FPS91_TY_S_MR_2BLAK554.GetValue("MABPCODE").ToString().Substring(1, 1).ToString();
            fsLARGE   = this.FPS91_TY_S_MR_2BLAK554.GetValue("MABPCODE").ToString().Substring(2, 2).ToString();
            fsMIDDLE  = this.FPS91_TY_S_MR_2BLAK554.GetValue("MABPCODE").ToString().Substring(4, 4).ToString();
            fsSMALL   = this.FPS91_TY_S_MR_2BLAK554.GetValue("MABPCODE").ToString().Substring(8, 3).ToString();
            fsJASANNM = this.FPS91_TY_S_MR_2BLAK554.GetValue("FXSMDESC").ToString();
