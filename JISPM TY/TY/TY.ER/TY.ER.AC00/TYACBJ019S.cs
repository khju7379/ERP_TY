...
namespace TY.ER.AC00
{
    ...
    /// 
    /// # 프로시저 정보 ####
    ///  TY_P_AC_2615V787 : K-IFRS 손익계산서 조회 및 출력
    ///  TY_P_AC_2615V788 : K-IFRS 손익계산서 조회 및 출력(IS 세목 포함)
    ...
    ///  GCUNO : 기준년월-기
    /// </summary>
    public partial class TYACBJ019S : TYBase
    {
        ...
        private void TYACBJ019S_Load(object sender, System.EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (this.CKB01_AG1YNISDT.GetValue().ToString() == "Y")
                {
                    ...
                    if (ISdt.Rows.Count > 0)
                    {
                        ...
                        SectionReport ISrpt = new TYACBJ019R();
                ...
                {
                    SectionReport rpt = new TYACBJ019R();
        ...
        private DataTable UP_ConvertDt(DataTable dt, DataTable ISdt, string sGUBUN)
        {
            ...
            double dHR매출액     = 0; // (1) 41000000
            double dHR매출원가   = 0; // (2) 42000000
            double dHR매출총이익 = 0; // (3) = (1) - (2) 43000000

            double dHR판관비     = 0; // (4) 44000000
            double dHR기타수익   = 0; // (5) 45000000
            double dHR기타비용   = 0; // (6) 46000000
            double dHR영업이익   = 0; // (7) = (3) - (4) + (5) - (6) 47000000

            double dHR금융수익              = 0; // (8) 48000000
            double dHR금융원가              = 0; // (9) 49000000
            double dHR종속관계기업투자손익   = 0; // (10) 50000000
            double dHR법인세비용차감전순이익 = 0; // (11) = (7) + (8) - (9) + (10) 51000000

            double dHR법인세비용 = 0; // (12) 52000000
            double dHR당기순이익 = 0; // (13) = (11) - (12) 53000000

            double dHR기타포괄순익   = 0; // (14) 54000000
            double dHR당기총포괄순익 = 0; // (15) = (13) + (14) 55000000

            double dOR매출액     = 0; // (1) 41000000
            double dOR매출원가   = 0; // (2) 42000000
            double dOR매출총이익 = 0; // (3) = (1) - (2) 43000000

            double dOR판관비     = 0; // (4) 44000000
            double dOR기타수익   = 0; // (5) 45000000
            double dOR기타비용   = 0; // (6) 46000000
            double dOR영업이익   = 0; // (7) = (3) - (4) + (5) - (6) 47000000

            double dOR금융수익              = 0; // (8) 48000000
            double dOR금융원가              = 0; // (9) 49000000
            double dOR종속관계기업투자손익   = 0; // (10) 50000000
            double dOR법인세비용차감전순이익 = 0; // (11) = (7) + (8) - (9) + (10) 51000000

            double dOR법인세비용 = 0; // (12) 52000000
            double dOR당기순이익 = 0; // (13) = (11) - (12) 53000000

            double dOR기타포괄순익   = 0; // (14) 54000000
            double dOR당기총포괄순익 = 0; // (15) = (13) + (14) 55000000
            ...
            for (int i = 0; i <= ConDt.Rows.Count - 1; i++)
            {
                ...
                {
                    dHR기타수익 = dHR기타수익 + double.Parse(ConDt.Rows[i]["HCRAMT"].ToString());
                    dOR기타수익 = dOR기타수익 + double.Parse(ConDt.Rows[i]["OCRAMT"].ToString());
                }
                else if (sA1CDAC.ToString() == "46000000")
                {
                    dHR기타비용 = dHR기타비용 + double.Parse(ConDt.Rows[i]["HCRAMT"].ToString());
                    dOR기타비용 = dOR기타비용 + double.Parse(ConDt.Rows[i]["OCRAMT"].ToString());
                }
                else if (sA1CDAC.ToString() == "47000000")
                {
                    dHR영업이익 = dHR매출총이익 - dHR판관비 + dHR기타수익 - dHR기타비용;
                    dOR영업이익 = dOR매출총이익 - dOR판관비 + dOR기타수익 - dOR기타비용;
                }
                else if (sA1CDAC.ToString() == "48000000")
                {
                    dHR금융수익 = double.Parse(ConDt.Rows[i]["HCRAMT"].ToString());
                    dOR금융수익 = double.Parse(ConDt.Rows[i]["OCRAMT"].ToString());
                }
                else if (sA1CDAC.ToString() == "49000000")
                {
                    dHR금융원가 = double.Parse(ConDt.Rows[i]["HCRAMT"].ToString());
                    dOR금융원가 = double.Parse(ConDt.Rows[i]["OCRAMT"].ToString());
                }
                else if (sA1CDAC.ToString() == "50000000")
                {
                    dHR종속관계기업투자손익 = double.Parse(ConDt.Rows[i]["HCRAMT"].ToString());
                    dOR종속관계기업투자손익 = double.Parse(ConDt.Rows[i]["OCRAMT"].ToString());
                }
                else if (sA1CDAC.ToString() == "51000000")
                {
                    dHR법인세비용차감전순이익 = dHR영업이익 + dHR금융수익 - dHR금융원가 + dHR종속관계기업투자손익;
                    dOR법인세비용차감전순이익 = dOR영업이익 + dOR금융수익 - dOR금융원가 + dOR종속관계기업투자손익;
                }
                ...
                else if (sA1CDAC.ToString() == "53000000")
                {
                    dHR당기순이익 = dHR법인세비용차감전순이익 - dHR법인세비용;
                    dOR당기순이익 = dOR법인세비용차감전순이익 - dOR법인세비용;
                }
                else if (sA1CDAC.ToString() == "54000000")
                {
                    dHR기타포괄순익 = double.Parse(ConDt.Rows[i]["HCRAMT"].ToString());
                    dOR기타포괄순익 = double.Parse(ConDt.Rows[i]["OCRAMT"].ToString());
                }
                else if (sA1CDAC.ToString() == "55000000")
                {
                    dHR당기총포괄순익 = dHR당기순이익 + dHR기타포괄순익;
                    dOR당기총포괄순익 = dOR당기순이익 + dOR기타포괄순익;
                }

                if (
                      (sA1CDAC.ToString() != "43000000" && sA1CDAC.ToString() != "47000000" &&
                       sA1CDAC.ToString() != "51000000" && sA1CDAC.ToString() != "53000000" &&
                       sA1CDAC.ToString() != "55000000") 
                       &&
                      (
                ...
                else
                {
                    ...
                    {
                        row["HCRAMT"] = string.Format("{0:#,###}", dHR당기총포괄순익);
                        row["OCRAMT"] = string.Format("{0:#,###}", dOR당기총포괄순익);
                    }
                    else
                    {
