...
{
    /// <summary>
    /// 기타예산 세목별 예산 명세서 프로그램입니다.
    /// 
    /// 작성자 : 김상권
    /// 작성일 : 2012.09.05 13:55
    /// 
    /// =====================================================================================
    ...
    /// 
    /// # 프로시저 정보 ####
    ///  TY_P_AC_2953B821 : 기타예산세목예산명세서
    /// 
    ///  # 스프레드 정보 ####
    ///  TY_S_AC_2954K825 : 기타예산세목예산명세서
    /// 
    ///  # 알림문자 정보 ####
    ...
    ///  GDATE : 일자
    /// </summary>
    public partial class TYACLB016S : TYBase
    {
        ...
        private void TYACLB016S_Load(object sender, System.EventArgs e)
        {
        ...
        private void BTN61_INQ_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                this.FPS91_TY_S_AC_2954K825.SetValue(UP_ConvertDt(dt));

                // 특정 ROW 색깔 입히기
                for (int i = 0; i < this.FPS91_TY_S_AC_2954K825.ActiveSheet.RowCount; i++)
                {
                    if (this.FPS91_TY_S_AC_2954K825.GetValue(i, "P2CDAC").ToString() == "계 정 계")
                    {
                        // 특정 ROW 색깔 입히기
                        this.FPS91_TY_S_AC_2954K825.ActiveSheet.Rows[i].BackColor = Color.FromArgb(228, 242, 194);
                    }
                    else if (this.FPS91_TY_S_AC_2954K825.GetValue(i, "P2CDAC").ToString() == "부 서 계")
                    {
                        // 특정 ROW 색깔 입히기
                        this.FPS91_TY_S_AC_2954K825.ActiveSheet.Rows[i].BackColor = Color.FromArgb(218, 239, 244);
                    }
                    else if (this.FPS91_TY_S_AC_2954K825.GetValue(i, "P2CDAC").ToString() == "총     계")
                    {
                        // 특정 ROW 색깔 입히기
                        this.FPS91_TY_S_AC_2954K825.ActiveSheet.Rows[i].BackColor = Color.FromArgb(254, 209, 164);
                    }
                }
            }
            else
        ...
        private void BTN61_PRT_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                SectionReport rpt1 = new TYACLB016R();
        ...
        private DataTable UP_ConvertDt(DataTable dt)
        {
            ...
            for (i = 1; i < nNum; i++)
            {
                if (table.Rows[i - 1]["P2CDDP"].ToString() != table.Rows[i]["P2CDDP"].ToString())
                {
                    // 소계
                    row = table.NewRow();
                    table.Rows.InsertAt(row, i);

                    table.Rows[i]["GUBUN"]   = table.Rows[i - 1]["GUBUN"].ToString();
                    table.Rows[i]["YESAN"]   = table.Rows[i - 1]["YESAN"].ToString();
                    table.Rows[i]["P2CDDP"]  = table.Rows[i - 1]["P2CDDP"].ToString();
                    ...
                    // 소 계 이름 넣기
                    table.Rows[i]["P2CDAC"] = "계 정 계";

                    sCDAC = "CDAC = '" + table.Rows[i - 1]["CDAC"].ToString() + "' ";
                    sCDAC += " AND P2CDDP = '" + table.Rows[i - 1]["P2CDDP"].ToString() + "'  ";
                    ...
                    table.Rows[i + 1]["GUBUN"]   = table.Rows[i - 2]["GUBUN"].ToString();
                    table.Rows[i + 1]["YESAN"]   = table.Rows[i - 2]["YESAN"].ToString();
                    table.Rows[i + 1]["P2CDDP"]  = table.Rows[i - 2]["P2CDDP"].ToString();
            ...
            Condt.Columns.Add("P2SEQ",   typeof(System.String));
            ...
            for (i = 0; i < table.Rows.Count; i++)
            {
                ...
                row["P2SEQ"]  = table.Rows[i]["P2SEQ"].ToString();
