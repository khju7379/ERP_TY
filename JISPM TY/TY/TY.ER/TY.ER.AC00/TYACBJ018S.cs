...
namespace TY.ER.AC00
{
    ...
    /// 
    /// # 프로시저 정보 ####
    ///  TY_P_AC_25U6Q738 : K-GAAP 대차대조표 조회 및 출력
    ///  TY_P_AC_25U6S739 : K-GAAP 대차대조표 조회 및 출력(BS 세목)
    ...
    ///  GCUNO     : 기준년월-기
    /// </summary>
    public partial class TYACBJ018S : TYBase
    {
        ...
        private void TYACBJ018S_Load(object sender, System.EventArgs e)
        {
        ...
        private void BTN61_PRT_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                SectionReport rpt = new TYACBJ014R();
        ...
        private DataTable UP_ConvertDt(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (Convert.ToString(dr["A1CDAC"]) == "33070200")
                {
            ...
            for (int i = 0; i <= ConDt.Rows.Count - 1; i++)
            {
                ...
                if (sNEWA1CDAC.ToString() == "33070200")
                {
                ...
                else
                {
                    ...
                    if (ConDt.Rows[i]["A1CDAC"].ToString().Substring(1, 7) == "0000000")
                    {
                        ...
                        if (sOLDA1CDAC != "" && (sNEWA1CDAC != sOLDA1CDAC))
                        {
                            ...
                            row["GSTDATE"] = _fsGSTDATE.ToString();
                            row["GEDDATE"] = _fsGEDDATE.ToString();
                            row["A1CDAC"]  = sOLDA1CDAC.ToString().Substring(0,1) + "9999999";
                            row["A1LVAC"]  = "1";
            ...
            row["GSTDATE"] = _fsGSTDATE.ToString();
            row["GEDDATE"] = _fsGEDDATE.ToString();
            row["A1CDAC"]  = sOLDA1CDAC.ToString().Substring(0, 1) + "9999995";
            row["A1LVAC"]  = "9";
            row["A1ABAC"]  = "(당기순이익 : ";
            row["A1TAG10"] = "9";
            row["A1YNBS"]  = "Y";
            ...
            row["GSTDATE"] = _fsGSTDATE.ToString();
            row["GEDDATE"] = _fsGEDDATE.ToString();
            row["A1CDAC"]  = sOLDA1CDAC.ToString().Substring(0, 1) + "9999996";
            row["A1LVAC"]  = "9";
            ...
            row["GSTDATE"] = _fsGSTDATE.ToString();
            row["GEDDATE"] = _fsGEDDATE.ToString();
            row["A1CDAC"]  = sOLDA1CDAC.ToString().Substring(0, 1) + "9999997";
            row["A1LVAC"]  = "9";
            ...
            row["GSTDATE"] = _fsGSTDATE.ToString();
            row["GEDDATE"] = _fsGEDDATE.ToString();
            row["A1CDAC"]  = sOLDA1CDAC.ToString().Substring(0, 1) + "9999998";
            row["A1LVAC"]  = "1";
            row["A1ABAC"]  = sOLDA1ABAC.ToString() + "총계";
            row["A1TAG10"] = "10";
            row["A1YNBS"]  = "Y";
            ...
            Finaldt.Columns.Add("GSTDATE", typeof(System.String));
            Finaldt.Columns.Add("GEDDATE", typeof(System.String));
            Finaldt.Columns.Add("A1CDAC",  typeof(System.String));
            Finaldt.Columns.Add("A1LVAC",  typeof(System.String));
            Finaldt.Columns.Add("A1ABAC",  typeof(System.String));
            Finaldt.Columns.Add("A1TAG10", typeof(System.String));
            ...
            Finaldt.Columns.Add("CUNO",    typeof(System.String));
            Finaldt.Columns.Add("BENO",    typeof(System.String));
            ...
            foreach (DataRow dr in Retdt.Rows)
            {
                ...
                row["GSTDATE"] = dr["GSTDATE"].ToString();
                row["GEDDATE"] = dr["GEDDATE"].ToString();
            ...
            DataTable RetBSdt = new DataTable();

            // B/S 계정세목
            if (this.CKB01_AG1YNBSDT.GetValue().ToString() == "Y")
            {
                this.DbConnector.CommandClear();
                ...
                DataTable BSdt = this.DbConnector.ExecuteDataTable();

                if (BSdt.Rows.Count > 0)
                {
                    RetBSdt = UP_Convert_BS_Dt(BSdt);

                    foreach (DataRow dr in RetBSdt.Rows)
                    {
                        row = Finaldt.NewRow();
            ...
            // 구조만 복사
            DataTable FinalBSdt = Finaldt.Clone();

            // 정렬 후 FinalBSdt 데이터테이블에 넣음.
            foreach (DataRow dr in Finaldt.Select("", "A1CDAC ASC"))
            {
                FinalBSdt.Rows.Add(dr.ItemArray);
            }
        ...
        #region Description : BS세목 데이터테이블 컨버젼
        private DataTable UP_Convert_BS_Dt(DataTable dt)
        {
            DataTable Retdt   = new DataTable();
