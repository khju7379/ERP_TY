...
{
    /// <summary>
    /// 여비교통비세목예산명세서 프로그램입니다.
    /// 
    /// 작성자 : 김상권
    /// 작성일 : 2012.09.06 09:41
    /// 
    /// =====================================================================================
    ...
    /// 
    /// # 프로시저 정보 ####
    ///  TY_P_AC_2969Z851 : 여비교통비세목예산명세서
    /// 
    ///  # 스프레드 정보 ####
    ///  TY_S_AC_296A1852 : 여비교통비세목예산명세서
    /// 
    ///  # 알림문자 정보 ####
    ...
    ///  GDATE : 일자
    /// </summary>
    public partial class TYACLB017S : TYBase
    {
        ...
        private void TYACLB017S_Load(object sender, System.EventArgs e)
        {
        ...
        private void BTN61_INQ_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                this.FPS91_TY_S_AC_296A1852.SetValue(UP_ConvertDt(dt));

                // 특정 ROW 색깔 입히기
                for (int i = 0; i < this.FPS91_TY_S_AC_296A1852.ActiveSheet.RowCount; i++)
                {
                    if (this.FPS91_TY_S_AC_296A1852.GetValue(i, "T2CDAC").ToString() == "계 정 계")
                    {
                        // 특정 ROW 색깔 입히기
                        this.FPS91_TY_S_AC_296A1852.ActiveSheet.Rows[i].BackColor = Color.FromArgb(228, 242, 194);
                    }
                    else if (this.FPS91_TY_S_AC_296A1852.GetValue(i, "T2CDAC").ToString() == "부 서 계")
                    {
                        // 특정 ROW 색깔 입히기
                        this.FPS91_TY_S_AC_296A1852.ActiveSheet.Rows[i].BackColor = Color.FromArgb(218, 239, 244);
                    }
                    else if (this.FPS91_TY_S_AC_296A1852.GetValue(i, "T2CDAC").ToString() == "총     계")
                    {
                        // 특정 ROW 색깔 입히기
                        this.FPS91_TY_S_AC_296A1852.ActiveSheet.Rows[i].BackColor = Color.FromArgb(254, 209, 164);
                    }
                }
            }
            else
        ...
        private void BTN61_PRT_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                SectionReport rpt1 = new TYACLB017R();
        ...
        private DataTable UP_ConvertDt(DataTable dt)
        {
            ...
            for (i = 1; i < nNum; i++)
            {
                if (table.Rows[i - 1]["T2CDDP"].ToString() != table.Rows[i]["T2CDDP"].ToString())
                {
                    // 소계
                    row = table.NewRow();
                    table.Rows.InsertAt(row, i);

                    table.Rows[i]["YESAN"]   = table.Rows[i - 1]["YESAN"].ToString();
                    table.Rows[i]["T2CDDP"]  = table.Rows[i - 1]["T2CDDP"].ToString();
                    ...
                    table.Rows[i]["A1ABAC"]  = table.Rows[i - 1]["A1ABAC"].ToString();
                    table.Rows[i]["CDDESC1"] = table.Rows[i - 1]["CDDESC1"].ToString();

                    // 소 계 이름 넣기
                    table.Rows[i]["T2CDAC"] = "계 정 계";

                    sCDAC = "CDAC = '" + table.Rows[i - 1]["CDAC"].ToString() + "' ";
                    sCDAC += " AND T2CDDP = '" + table.Rows[i - 1]["T2CDDP"].ToString() + "'  ";
                    ...
                    table.Rows[i + 1]["YESAN"]   = table.Rows[i - 2]["YESAN"].ToString();
                    table.Rows[i + 1]["T2CDDP"]  = table.Rows[i - 2]["T2CDDP"].ToString();
                    table.Rows[i + 1]["BUSEO"]   = table.Rows[i - 2]["BUSEO"].ToString();
                    table.Rows[i + 1]["DATE"]    = table.Rows[i - 2]["DATE"].ToString();
                    table.Rows[i + 1]["CDAC"]    = table.Rows[i - 2]["CDAC"].ToString();
                    table.Rows[i + 1]["A1ABAC"]  = table.Rows[i - 2]["A1ABAC"].ToString();
                    table.Rows[i + 1]["CDDESC1"] = table.Rows[i - 2]["CDDESC1"].ToString();

                    // 사업부 소계 이름 넣기
                    table.Rows[i + 1]["T2CDAC"] = "부 서 계";

                    table.Rows[i + 1]["AMT01"] = string.Format("{0:###0}", dAMT1_SA);
                    table.Rows[i + 1]["AMT02"] = string.Format("{0:###0}", dAMT2_SA);
                    table.Rows[i + 1]["AMT03"] = string.Format("{0:###0}", dAMT3_SA);
                    table.Rows[i + 1]["AMT04"] = string.Format("{0:###0}", dAMT4_SA);
                    table.Rows[i + 1]["AMT05"] = string.Format("{0:###0}", dAMT5_SA);
                    table.Rows[i + 1]["AMT06"] = string.Format("{0:###0}", dAMT6_SA);
                    table.Rows[i + 1]["AMT07"] = string.Format("{0:###0}", dAMT7_SA);
                    table.Rows[i + 1]["AMT08"] = string.Format("{0:###0}", dAMT8_SA);
                    table.Rows[i + 1]["AMT09"] = string.Format("{0:###0}", dAMT9_SA);
            ...
            Condt.Columns.Add("YESAN",   typeof(System.String));
            Condt.Columns.Add("T2CDDP",  typeof(System.String));
            Condt.Columns.Add("BUSEO",   typeof(System.String));
            Condt.Columns.Add("CDAC",    typeof(System.String));
            Condt.Columns.Add("T2CDAC",  typeof(System.String));
            Condt.Columns.Add("A1ABAC",  typeof(System.String));
            Condt.Columns.Add("T2SEQ",   typeof(System.String));
            Condt.Columns.Add("CDDESC1", typeof(System.String));
            Condt.Columns.Add("T2CDSB",  typeof(System.String));
            Condt.Columns.Add("KBHANGL", typeof(System.String));
            Condt.Columns.Add("T1RKAC",  typeof(System.String));
            ...
            for (i = 0; i < table.Rows.Count; i++)
            {
                ...
                row["YESAN"]   = table.Rows[i]["YESAN"].ToString();
                row["T2CDDP"]  = table.Rows[i]["T2CDDP"].ToString();
                row["BUSEO"]   = table.Rows[i]["BUSEO"].ToString();
                row["CDAC"]    = table.Rows[i]["CDAC"].ToString();
                row["T2CDAC"]  = table.Rows[i]["T2CDAC"].ToString();
                row["T2SEQ"]   = table.Rows[i]["T2SEQ"].ToString();
                row["T1RKAC"]  = table.Rows[i]["T1RKAC"].ToString();
                row["T2CDSB"]  = table.Rows[i]["T2CDSB"].ToString();
