...
namespace TY.ER.UT00
{
    ...
    ///  CHYMSEQ : 순번
    /// </summary>
    public partial class TYUTME014B : TYBase
    {
        ...
        private void TYUTME014B_Load(object sender, System.EventArgs e)
        {
            this.BTN61_CREATE.ProcessCheck += new TButton.CheckHandler(BTN61_CREATE_ProcessCheck);

            this.DTP01_JBDATE.SetValue(DateTime.Now.ToString("yyyy-MM-dd"));
            this.DTP01_JBIPHANG.SetValue("");

            SetStartingFocus(this.DTP01_JBDATE);
        }
        #endregion
        ...
        private void BTN61_CREATE_Click(object sender, EventArgs e)
        {
            ...
            // 선급금 전표생성 SP 수행
            this.DbConnector.CommandClear();
            this.DbConnector.Attach("TY_P_UT_73KGK001", sB2SSID.ToString(),
                                                        sB2SSID.Length,
                                                        Get_Date(this.DTP01_JBDATE.GetValue().ToString()),
                                                        Get_Date(this.DTP01_JBIPHANG.GetValue().ToString()),
                                                        this.CBH01_JBBRANCH.GetValue().ToString(),
        ...
        private void BTN61_PRT_Click(object sender, EventArgs e)
        {
            ...
            this.DbConnector.CommandClear();
            this.DbConnector.Attach("TY_P_UT_73KH9002", Get_Date(this.DTP01_JBDATE.GetValue().ToString()),
                                                        Get_Date(this.DTP01_JBIPHANG.GetValue().ToString()),
                                                        this.CBH01_JBBRANCH.GetValue().ToString(),
                                                        this.TXT01_JBSEQ.GetValue().ToString());
        ...
        private void BTN61_CREATE_ProcessCheck(object sender, TButton.ClickEventCheckArgs e)
        {
            ...
            if (this.CBO01_GGUBUN.GetValue().ToString() == "A")
            {
                // 전표 존재유무 체크
                this.DbConnector.CommandClear();
                this.DbConnector.Attach("TY_P_UT_73KH5003", Get_Date(this.DTP01_JBDATE.GetValue().ToString()),
                                                            Get_Date(this.DTP01_JBIPHANG.GetValue().ToString()),
                                                            this.CBH01_JBBRANCH.GetValue().ToString(),
                                                            this.TXT01_JBSEQ.GetValue().ToString());
                ...
                {
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_UT_73KHA004", Get_Date(this.DTP01_JBDATE.GetValue().ToString()),
                                                                Get_Date(this.DTP01_JBIPHANG.GetValue().ToString()),
                                                                this.CBH01_JBBRANCH.GetValue().ToString(),
                                                                this.TXT01_JBSEQ.GetValue().ToString());
                    ...
                    else
                    {
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            // UTT 거래처 확인
                            this.DbConnector.CommandClear();
                            this.DbConnector.Attach("TY_P_UT_669GM112", dt.Rows[i]["JBBRANCH"].ToString().Trim());
                            ...
                            if (dt1.Rows.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            if (double.Parse(dt.Rows[i]["JBRATE"].ToString().Trim()) == 0)
                                            {
                                            ...
                                            if (double.Parse(dt.Rows[i]["JBDOLLAR"].ToString().Trim()) == 0)
                                            {
            ...
            else
            {
                // 전표 존재유무 체크
                this.DbConnector.CommandClear();
                this.DbConnector.Attach("TY_P_UT_73KHK005", Get_Date(this.DTP01_JBDATE.GetValue().ToString()),
                                                            Get_Date(this.DTP01_JBIPHANG.GetValue().ToString()),
                                                            this.CBH01_JBBRANCH.GetValue().ToString(),
                                                            this.TXT01_JBSEQ.GetValue().ToString());
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        this.DbConnector.CommandClear();
                        this.DbConnector.Attach("TY_P_UT_73KHL006", dt.Rows[i]["JBJPNO"].ToString().Substring(0, 6).ToString(),
                                                                    dt.Rows[i]["JBJPNO"].ToString().Substring(6, 8).ToString(),
                                                                    dt.Rows[i]["JBJPNO"].ToString().Substring(14, 3).ToString());
                        ...
                        this.DbConnector.CommandClear();
                        this.DbConnector.Attach("TY_P_UT_C3SBF197", "6108110449", dt.Rows[i]["JBJPNO"].ToString());
