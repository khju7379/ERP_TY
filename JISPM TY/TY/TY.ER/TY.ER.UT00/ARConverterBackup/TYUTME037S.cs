...
namespace TY.ER.UT00
{
    ...
    ///  SDATE : 시작일자
    /// </summary>
    public partial class TYUTME037S : TYBase
    {
        ...
        private void TYUTME037S_Load(object sender, System.EventArgs e)
        {
        ...
        private void BTN61_INQ_Click(object sender, EventArgs e)
        {
            ...
            this.FPS91_TY_S_UT_7CCHL258.Initialize();

            this.DbConnector.CommandClear();

            this.DbConnector.Attach("TY_P_UT_7CCHK257", Get_Date(this.DTP01_SDATE.GetValue().ToString()),
                                                        this.CBH01_SHWAJU.GetValue().ToString()
                                                        );

            dt = this.DbConnector.ExecuteDataTable();

            this.FPS91_TY_S_UT_7CCHL258.SetValue(dt);

            if (this.FPS91_TY_S_UT_7CCHL258.CurrentRowCount > 0)
            {
                for (int i = 0; i < this.FPS91_TY_S_UT_7CCHL258.CurrentRowCount; i++)
                {
                    if (this.FPS91_TY_S_UT_7CCHL258.GetValue(i, "VNSANGHO").ToString() == "[합 계]")
                    {
                        // 특정 ROW 글자 크기 변경
                        this.FPS91_TY_S_UT_7CCHL258.ActiveSheet.Rows[i].Font = new Font("굴림", 9, FontStyle.Bold);

                        // 특정 칼럼 색깔 입히기
                        this.FPS91_TY_S_UT_7CCHL258.ActiveSheet.Rows[i].BackColor = Color.SkyBlue;
                    }
                }
            }
        }
