...
namespace TY.ER.US00
{
    ...
    ///  CHYMSEQ : 순번
    /// </summary>
    public partial class TYUSME064P : TYBase
    {
        ...
        private void TYUSME064P_Load(object sender, System.EventArgs e)
        {
        ...
        private void BTN61_PRT_Click(object sender, EventArgs e)
        {
            ...
            if (this.CBH01_STHANGCHA.GetValue().ToString().Trim() == "" && this.CBH01_EDHANGCHA.GetValue().ToString().Trim() == "")
            {
                // 소급 매출 세금계산서 출력 - 항차(MIN, MAX) 가져오기
                this.DbConnector.CommandClear();
                this.DbConnector.Attach("TY_P_US_945E4251", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                ...
                if (dt.Rows.Count > 0)
                {
                    this.CBH01_STHANGCHA.SetValue(dt.Rows[0]["STHANGCHA"].ToString());
                    this.CBH01_EDHANGCHA.SetValue(dt.Rows[0]["EDHANGCHA"].ToString());
                }
            }
            ...
            if (this.CBO01_GGUBUN.GetText().ToString() == "전표")
            {
                ...
                // 현업 매출 전표 데이터 가져오기
                this.DbConnector.CommandClear();
                this.DbConnector.Attach("TY_P_US_945DM249", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                                                            this.CBO01_GMEGUBUN.GetValue().ToString().Trim(),
                                                            this.CBH01_GHWAJU.GetValue().ToString().Trim()
                                                            );
                ...
                if (dt_JPNO.Rows.Count > 0)
                {
                    ...
                    for (i = 0; i < dt_JPNO.Rows.Count; i++)
                    {
                        if (sB2NOSQ == "")
                        {
                            sB2NOSQ = dt_JPNO.Rows[i]["SGJPNO"].ToString().Substring(14, 3);
                        }
                        else
                        {
                            sB2NOSQ = sB2NOSQ + "," + dt_JPNO.Rows[i]["SGJPNO"].ToString().Substring(14, 3);
                        }
                    }
                    ...
                    if (Convert.ToDouble(sB2DTMK.ToString()) > 20200117)    // 조득원 대리 요청 2020-01-17
                    {
                    ...
                    else if (Convert.ToDouble(sB2DTMK.ToString().Substring(0, 4)) > 2014)
                    {
            ...
            else if (this.CBO01_GGUBUN.GetText().ToString() == "세금계산서")
            {
                ...
                this.DbConnector.CommandClear();
                this.DbConnector.Attach("TY_P_US_945E1250", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                ...
                if (dt.Rows.Count > 0)
                {
                    SectionReport rpt = new TYUSME017R();
                    // 세로 출력
                    rpt.PageSettings.Orientation = GrapeCity.ActiveReports.Document.Section.PageOrientation.Portrait;
            ...
            else if (this.CBO01_GGUBUN.GetText().ToString() == "거래명세서")
            {
                ...
                {
                    sProcedure = "TY_P_US_945EG252";
                }
                else if (this.CBO01_GMEGUBUN.GetText().ToString() == "하역료")
                {
                    sProcedure = "TY_P_US_945EK253";
                }
                ...
                if (dt.Rows.Count > 0)
                {
                    if (this.CBO01_GMEGUBUN.GetText().ToString() == "시설사용료")
                    {
                        SectionReport rpt = new TYUSME064R1();
                    ...
                    {
                        SectionReport rpt = new TYUSME064R2();
        ...
        private DataTable UP_Get_ConvertTax(DataTable Orgdt)
        {
            ...
            for (i = 0; i < Orgdt.Rows.Count; i++)
            {
                ...
                dtRow["VSDESC1"] = sVSDESC1.ToString();
