...
namespace TY.ER.US00
{
    ...
    ///  YYYYMM : 기준 년월
    /// </summary>
    public partial class TYUSME041B : TYBase
    {
        ...
        private void TYUSME041B_Load(object sender, System.EventArgs e)
        {
            this.DTP01_GSTYYMM.SetValue(System.DateTime.Now.ToString("yyyy-MM"));
            this.DTP01_GEDYYMM.SetValue(System.DateTime.Now.AddMonths(1).ToString("yyyy-MM"));

            this.DTP01_EDYYMM.SetValue(System.DateTime.Now.ToString("yyyy-MM"));

            this.FPS91_TY_S_US_92BHA716.Initialize();
            this.FPS91_TY_S_US_92BHB717.Initialize();
        ...
        private void BTN61_BATCH_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.DbConnector.Attach("TY_P_US_92BFW711",
                                        Get_Date(this.DTP01_GSTYYMM.GetValue().ToString()),
                                        Get_Date(this.DTP01_GEDYYMM.GetValue().ToString()),
        ...
        private void BTN61_INQ_Click(object sender, EventArgs e)
        {
            ...
            string sDate = this.DTP01_EDYYMM.GetValue().ToString().Substring(0, 4) + "-" + this.DTP01_EDYYMM.GetValue().ToString().Substring(4, 2) + "-01";
            ...
            // 전월 미수 조회
            this.DbConnector.CommandClear();
            this.DbConnector.Attach("TY_P_US_92BHJ719", Convert.ToString(dDate).ToString().Replace("-", "").Substring(0,6));
            dt = this.DbConnector.ExecuteDataTable();

            this.FPS91_TY_S_US_92BHA716.SetValue(dt);


            groupBox6.Text = this.DTP01_EDYYMM.GetValue().ToString().Substring(0, 4) + "년" + this.DTP01_EDYYMM.GetValue().ToString().Substring(4, 2) + "월";

            // 당월 미수 조회
            this.DbConnector.CommandClear();
            this.DbConnector.Attach("TY_P_US_92BHJ719", this.DTP01_EDYYMM.GetValue().ToString());
            dt = this.DbConnector.ExecuteDataTable();

            this.FPS91_TY_S_US_92BHB717.SetValue(dt);
        }
        #endregion
    }
}