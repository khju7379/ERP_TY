...
namespace TY.ER.US00
{
    ...
    ///  CHYMSEQ : 순번
    /// </summary>
    public partial class TYUSME015B : TYBase
    {
        ...
        private void TYUSME015B_Load(object sender, System.EventArgs e)
        {
        ...
        private void BTN61_CREATE_ProcessCheck(object sender, TButton.ClickEventCheckArgs e)
        {
            ...
            if (this.CBO01_GGUBUN.GetText().ToString() == "취소")
            {
                if (this.CBO01_GMEGUBUN.GetText().ToString() == "시설사용료")
                {
                    // 전표 존재유무 체크
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_US_92RDV930", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            // AHSLGLF 전표 존재 유무 체크
                            this.DbConnector.CommandClear();
                            this.DbConnector.Attach("TY_P_US_92RDL927", dt.Rows[i]["JPNO"].ToString().Substring(0, 6).ToString(),
                                                                        dt.Rows[i]["JPNO"].ToString().Substring(6, 8).ToString(),
                                                                        dt.Rows[i]["JPNO"].ToString().Substring(14, 3).ToString());
                            ...
                            this.DbConnector.CommandClear();
                            this.DbConnector.Attach("TY_P_US_C3SBD196", "6108110449", dt.Rows[i]["JPNO"].ToString());
                ...
                else if (this.CBO01_GMEGUBUN.GetText().ToString() == "하역료")
                {
                    // 전표 존재유무 체크
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_US_92RDX931", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                                                                this.CBH01_STHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_EDHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_GGOKJONG.GetValue().ToString().Trim(),
                                                                this.CBH01_GHWAJU.GetValue().ToString().Trim()
                ...
                else if (this.CBO01_GMEGUBUN.GetText().ToString() == "보관료")
                {
                    // 전표 존재유무 체크
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_US_92RE2932", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                                                                this.CBH01_STHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_EDHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_GGOKJONG.GetValue().ToString().Trim(),
                                                                this.CBH01_GHWAJU.GetValue().ToString().Trim()
                ...
                else if (this.CBO01_GMEGUBUN.GetText().ToString() == "조출료")
                {
                    // 전표 존재유무 체크
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_US_92RE5933", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                                                                this.CBH01_STHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_EDHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_GGOKJONG.GetValue().ToString().Trim(),
                                                                this.CBH01_GHWAJU.GetValue().ToString().Trim()
                ...
                else if (this.CBO01_GMEGUBUN.GetText().ToString() == "이송료")
                {
                    // 전표 존재유무 체크
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_US_92REA934", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                                                                this.CBH01_STHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_EDHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_GGOKJONG.GetValue().ToString().Trim(),
                                                                this.CBH01_GHWAJU.GetValue().ToString().Trim()
                ...
                else if (this.CBO01_GMEGUBUN.GetText().ToString() == "난작업비")
                {
                    // 전표 존재유무 체크
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_US_92RED935", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                                                                this.CBH01_STHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_EDHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_GGOKJONG.GetValue().ToString().Trim(),
                                                                this.CBH01_GHWAJU.GetValue().ToString().Trim()
                ...
                else if (this.CBO01_GMEGUBUN.GetText().ToString() == "별첨화주")
                {
                    // 전표 존재유무 체크
                    this.DbConnector.CommandClear();
                    this.DbConnector.Attach("TY_P_US_92REG936", Get_Date(this.MTB01_GDATE.GetValue().ToString()).Trim(),
                                                                this.CBH01_STHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_EDHANGCHA.GetValue().ToString().Trim(),
                                                                this.CBH01_GGOKJONG.GetValue().ToString().Trim(),
                                                                this.CBH01_GHWAJU.GetValue().ToString().Trim(),
